...
const TalentCard = ({ 
  ...
  const isAvailable = talent.availability !== false; // Default to true if not specified
  ...
  return (
    <div 
      className={`bg-[--color-primary-50] shadow-md text-center transition-all hover:shadow-lg hover:-translate-y-1 flex flex-col items-center
        px-4 py-6 w-[240px] h-[420px] rounded-[25px]
        xs:px-5 xs:py-7 xs:w-[280px] xs:h-[460px] xs:rounded-[30px]
        sm:px-6 sm:py-8 sm:w-[320px] sm:h-[520px] sm:rounded-[35px]
        md:px-8 md:py-10 md:w-[380px] md:h-[620px] md:rounded-[45px]
        lg:px-10 lg:py-12 lg:w-[420px] lg:h-[680px] lg:rounded-[50px]
        xl:px-11 xl:py-14 xl:w-[460px] xl:h-[740px] xl:rounded-[55px]
        2xl:px-12 2xl:py-16 2xl:w-[488px] 2xl:h-[790px] 2xl:rounded-[60.8px] ${className}`}
      {...props}
    >
      {/* Profile Image */}
      ...
      <h2 className="font-bold text-gray-900
                   text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl 2xl:text-h0">
        {talent.name}
      </h2>
      ...
      {showTitle && (
        <p className="text-[--color-primary-500] font-medium
                    ...
                    mb-1 xs:mb-2 sm:mb-2 md:mb-3 lg:mb-3 xl:mb-4 2xl:mb-4">
          Web Developer
        </p>
      )}
      ...
      {showDescription && (
        ...
        mb-3 xs:mb-4 sm:mb-5 md:mb-6 lg:mb-8 xl:mb-10 2xl:mb-12">
          {talent.description || 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam euismod lobortis diam, nec bibendum ipsum tincidunt ut.'}
        </p>
      )}

      {/* Skills */}
      {showSkills && skills.length > 0 && (
        <div className="flex flex-wrap justify-center gap-2 mb-4 px-2 w-full">
      ...
      {(showPortfolioButton || showHireButton) && (
        <div className="flex justify-center gap-1 xs:gap-2 sm:gap-3 md:gap-4 lg:gap-5 xl:gap-6 2xl:gap-6 mt-auto">
          {showPortfolioButton && (
            <Link
          ...
          {showHireButton && (
            <button
              className={`font-medium flex items-center justify-center rounded-lg transition
                       text-xs py-1 w-[60px] h-[24px]
                       xs:text-xs xs:py-1 xs:w-[70px] xs:h-[28px]
                       sm:text-sm sm:py-2 sm:w-[90px] sm:h-[32px]
                       md:text-sm md:py-2 md:w-[110px] md:h-[36px]
